...
function ReportsContent() {
  ...
  // Load classrooms + students on mount
  useEffect(() => {
    async function load() {
      try {
        ...
        if (preselectedStudentId) {
          ...
          if (preStudent) setSelectedClassroom(preStudent.classroom_id);
        }
      } catch {
        // ignore
      } finally {
  ...
  // Auto-generate report when student is pre-selected
  useEffect(() => {
  ...
  async function loadGradeReport(grade: number) {
    ...
    try {
      const headers = await getTeacherHeaders();
      const data = await apiFetch<GradeReport>(`/evaluator/report/grade/${grade}`, { headers });
      setGradeReport(data);
    } catch {
  ...
  const generateStudentReport = useCallback(async (studentId: string, range?: string) => {
    ...
    try {
      ...
      const data = await apiFetch<DetailedReport>(url, { headers });
      setReport(data);
      setView("student");
    } catch {
