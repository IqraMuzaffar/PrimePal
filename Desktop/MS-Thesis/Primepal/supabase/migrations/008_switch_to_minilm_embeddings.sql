-- 008_switch_to_minilm_embeddings.sql
-- Switches embedding model from OpenAI text-embedding-3-small (1536 dims)
-- to sentence-transformers/all-MiniLM-L6-v2 (384 dims, free, local).
--
...
-- 2. Clear existing rows (old 1536-dim vectors are incompatible with 384-dim column)
TRUNCATE TABLE snc_knowledge_base;

-- 3. Change the embedding column from 1536 to 384 dimensions
...
-- 5. Replace the RPC function with the 384-dim signature
