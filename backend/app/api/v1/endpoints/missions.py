...
async def get_daily_missions(
    ...
    # ------------------------------------------------------------------
    cache_key = make_cache_key("daily_missions", student_id, classroom_id, str(is_frustrated), topics_hash)
...
async def _generate_personalized_missions(
    ...
    try:
        ...
        from app.agents.tutor_agent.question_validator import merge_bank_and_llm
        merged = merge_bank_and_llm(bank_qs or [], llm_qs or [], pillar, PILLAR_QUESTIONS_COUNT)
        ...
        if len(merged) < PILLAR_QUESTIONS_COUNT:
            ...
            if fallback:
                merged = merge_bank_and_llm(merged, fallback, pillar, PILLAR_QUESTIONS_COUNT)
...
async def get_pillar_missions(
    ...
    # ------------------------------------------------------------------
    # Step 6: Merge bank(5) + LLM(5) = 10 questions (with deduplication)
    # ------------------------------------------------------------------
    from app.agents.tutor_agent.question_validator import merge_bank_and_llm
