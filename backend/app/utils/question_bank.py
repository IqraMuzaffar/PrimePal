...
def _shuffle_within_tiers(rows: list[dict], limit: int) -> list[dict]:
    """Shuffle rows that share the same times_served value, then take `limit`."""
    if not rows:
        return rows
    from itertools import groupby
    tiers = []
    for _, group in groupby(rows, key=lambda r: r.get("times_served", 0)):
        tier = list(group)
        random.shuffle(tier)
        tiers.extend(tier)
    return tiers[:limit]
...
async def _query_slot(
    ...
    Over-fetches 3x then shuffles within same times_served tier to avoid
    deterministic ordering when counters are equal.
    """
    try:
        over_fetch = limit * 3
        query = (
        ...
        resp = await asyncio.to_thread(
            lambda q=query: q.order("times_served").limit(over_fetch).execute()
        )
        rows = resp.data or []
        ...
        if len(rows) < limit and difficulty_pref:
            ...
            resp = await asyncio.to_thread(
                lambda q=fallback: q.order("times_served").limit(over_fetch).execute()
            )
            rows = resp.data or []

        rows = _shuffle_within_tiers(rows, limit)
        return rows
    except Exception as exc:
...
async def pull_from_bank(
    ...
    try:
        ...
        rows = _shuffle_within_tiers(rows, len(rows))
