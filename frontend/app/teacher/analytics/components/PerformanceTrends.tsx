...
import { TeacherDailyTrend } from "@/types/analytics";

interface PerformanceTrendsProps {
  trends: TeacherDailyTrend[];
  pillar?: string;
}
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-bold text-gray-900">Daily Performance Trends</h2>
        </div>
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-end gap-1 h-32">
            {trends.map((day, idx) => {
              const height = (day.avg_accuracy / maxAccuracy) * 100;
              return (
                <div key={idx} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex items-end justify-center h-24">
                    <div
                      ...
                      style={{ height: `${height}%` }}
                      title={`${day.date_label}: ${day.avg_accuracy}%`}
                    />
                  </div>
        ...
        <div>
          ...
          <div className="flex items-end gap-1 h-32">
            {trends.map((day, idx) => {
              const height = (day.total_interactions / maxInteractions) * 100;
              return (
                <div key={idx} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex items-end justify-center h-24">
                    <div
                      ...
                      style={{ height: `${height}%` }}
                      title={`${day.date_label}: ${day.total_interactions} interactions`}
                    />
                  </div>
                  <span className="text-[10px] text-gray-900 font-semibold">
